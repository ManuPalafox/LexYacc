...
{
  "\"end of file\"", "error", "\"invalid token\"", "BIN", "$accept", "S",
  "OUT", YY_NULLPTR
};
...
#line 21 "binhex.y"
           {
        n = strlen((yyvsp[0].cadenaBinaria)) ;
        aumentarCantidadBit((yyvsp[0].cadenaBinaria), k) ;
        j = k - 1 ;
        for(i = n - 1; i >= 0; i--) {
            while(j >= 0) {
                segmentos[i][j] = (yyvsp[0].cadenaBinaria)[i];
                if(i == (j * 4) - 1) {
                    j-- ;
                }
...
#line 1130 "y.tab.c"
  ...
  case 3: /* OUT: BIN  */
#line 36 "binhex.y"
             {(yyval.cadenaBinaria) = (yyvsp[0].cadenaBinaria);}
#line 1136 "y.tab.c"
    break;
